...
public class UIPatches : BendyVRPatch
{
    ...
    //Video Player Scale
    [HarmonyPrefix]
    [HarmonyPatch(typeof(JoeyApartmentController), nameof(JoeyApartmentController.ShowCredits))]
    private static void ScaleVideo(JoeyApartmentController __instance)
    {
        __instance.m_VideoPlayer.aspectRatio = UnityEngine.Video.VideoAspectRatio.NoScaling;        
    }

    [HarmonyPostfix]
    [HarmonyPatch(typeof(JoeyApartmentController), nameof(JoeyApartmentController.InitOnComplete))]
    private static void ScaleVideo2(JoeyApartmentController __instance)
    {
        __instance.m_VideoPlayer.aspectRatio = UnityEngine.Video.VideoAspectRatio.NoScaling;
    }

    [HarmonyPostfix]
    [HarmonyPatch(typeof(CH5TheEndCredits), nameof(CH5TheEndCredits.InitOnComplete))]
    private static void ScaleVideo3(CH5TheEndCredits __instance)
    {
        __instance.m_VideoPlayer.aspectRatio = UnityEngine.Video.VideoAspectRatio.NoScaling;
    }
    ...
    //Credits Screen Fix
    [HarmonyPostfix]
    [HarmonyPatch(typeof(CreditsScreenController), nameof(CreditsScreenController.InitController))]
    private static void IncreaseBlocker(CreditsScreenController __instance)
    {
        Transform blocker = __instance.transform.Find("Black");
        if (blocker)
            blocker.localScale = new Vector3(10f, 10f, 10f);
    }
    ...
    {
        Transform blocker = __instance.transform.Find("Blocker");
