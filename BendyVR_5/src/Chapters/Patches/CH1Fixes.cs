...
public class CH1Fixes : BendyVRPatch
{
	/*
	[HarmonyPrefix]
	[HarmonyPatch(typeof(CH1TheatreController), nameof(CH1TheatreController.DOFlooding))]
	private static bool DODifferentFlooding(CH1TheatreController __instance, ref Sequence __result)
	{
		Sequence sequence = DOTween.Sequence();
		float num = 0f;
		float num2 = 4f;
		__instance.m_RaisingInk.tag = "Ink";
		//sequence.Insert(num, __instance.m_InkRenderer.material.DOFloat(1f, "_Cutout", num2).SetEase(Ease.InQuad));
		__instance.m_InkRenderer.material.shader.
		sequence.Insert(num, __instance.m_InkRenderer.material.DOFloat(1f, "_Cutout", 0.0f).SetEase(Ease.InQuad));
		num += num2;
		num2 += num2 / 2f;
		sequence.Insert(num, __instance.m_RaisingInk.DOLocalMoveY(__instance.m_RaisingInk.localPosition.y + 1f, num2).SetEase(Ease.InOutQuad));
		sequence.InsertCallback(num + num2 / 8f, delegate
		{
			__instance.m_RaisingInk.tag = "DeepInk";
		});
		__result = sequence;
		return false;
	}*/
